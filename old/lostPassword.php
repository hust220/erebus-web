...
if (isset($_POST['email'])) {
	...
	if ($rs = $user->search(array('email'), array($_POST['email'])))
	{
		...
		if (! $user->save()) {
	   	...
	   	$mail_subject = "iFold lost password: " . ucwords ($rs[0]['firstname']) . " " . ucwords ($rs[0]['lastname']);
	    $mail_message = ucwords($rs[0]['firstname']) . ' ' . ucwords($rs[0]['lastname']) . " requested the password reset for username: {$rs[0]['username']}" .
