...
$notifyCompleted = $_POST['notifyCompleted'];
...
$query = "SELECT * FROM $tableusers WHERE email='$email' AND userid!='$userid'";
...
$query="SELECT email,emailConfirmed FROM $tableusers WHERE userid='$userid'";
$result = mysql_query($query);
...
if ($oldemail != $email || $emailConfirmed != 1) {
	$query="UPDATE $tableusers SET emailConfirmed='0' WHERE userid='$userid'";
...
$query="UPDATE $tableusers SET firstname='$firstname', ".
	" lastname='$lastname',email='$email',organization='$organization', notifyCompleted='$notifyCompleted' ".
	"WHERE userid='$userid'";
//die($query);
