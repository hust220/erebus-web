...
#--------save the form data-------------------------#
$varlist=array('username','password','confirm_pass','firstname','lastname','email','organization');
...
$query="INSERT INTO $tableusers (username,password,firstname,lastname,email,organization,secret,level,lastlogin,ipaddr) ".
	"VALUES('$username','$passmd5','$firstname','$lastname','$email','$organization','$secret',1,NOW(),'$ipaddr') ";
$result = mysql_query($query) or die("failed to insert into user database");
$id = mysql_insert_id();

$query="UPDATE $tableusers SET emailApproved='1' WHERE userid='$id' ";
$result = mysql_query($query) or die("failed to update user database");

# send email to user
$secretmd5 = md5($secret);
$link = "http://" . $_SERVER['HTTP_HOST'] . '/erebus/emailconfirm.php'."?username=$username&key=".
        sha1(md5($email) . $secretmd5 );
...
$mail_subject = "Erebus new user registration notice";
$mail_message = "There is a new user registered to erebus server\r\n username: $username \r\n" .
...
mail("shir@email.unc.edu,dokh@med.unc.edu", $mail_subject, $mail_message, $mail_headers);
